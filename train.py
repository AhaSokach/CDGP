...
get_checkpoint_path2 = f'./saved_models/edge-{args.data}.pth'
best_model_path = get_checkpoint_path2
...
for i in range(args.n_runs):
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for batch in range(0, num_batch, 1):
            ...
            pred = cdgp.forward(sources_batch, destinations_batch, timestamps_batch, edge_idxs_batch, index,
                                )
            if (sum(target_batch) > 0):
                # print(index)
                # print(batch)
                # print(index[0].shape[0])
                # print(sum(target_batch!=0))
                back_num += index[0].shape[0]
                # print(back_num)
                update_num_all += sum(target_batch != 0)
                # print(update_num_all)
                ...
                if (update_num_all >= 50) or (accumulation_steps == 8):
                    # loss_batch = loss / 5
                    # loss_batch.backward()
                    # logger.info(f'batch{batch}: {update_num_all} {accumulation_steps}')
                    optimizer.step()
                    optimizer.zero_grad()
        ...
        val_results, all_pred, all_target, all_cas = eval_popularity_prediction(model=cdgp, criterion=criterion,
                                                                                data=val_data,
                                                                                # n_neighbors=NUM_NEIGHBORS,
        ...
        test_results, all_pred, all_target, all_cas = eval_popularity_prediction(model=cdgp, criterion=criterion,
                                                                                 data=test_data,
                                                                                 # n_neighbors=NUM_NEIGHBORS,
                                                                                 logger=logger,
                                                                                 device=device,
