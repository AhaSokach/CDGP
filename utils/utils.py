...
class NeighborFinder:
  ...
  def get_community_neighbor(self, source_nodes, timestamps, max_member_num =20, n_neighbors=20):
    """
    Given a list of users ids and relative cut times, extracts a sampled temporal neighborhood of each user in the list.
    ...
    for i, (source_node, timestamp) in enumerate(zip(source_nodes, timestamps)):
      ...
      if len(source_neighbors) > 0 and n_neighbors > 0:
        source_neighbors = source_neighbors.tolist()
        source_neighbors.append(source_node)    # all_neighbor and itself
        source_neighbors = np.unique(source_neighbors).tolist()
        neighbors_unique.extend(source_neighbors[-max_member_num:])
        neighbors_num.append(min(len(source_neighbors),max_member_num))
        ...
        neighbors[j, 0:len(source_neighbors)] = source_neighbors
        edge_times[i, n_neighbors - len(source_edge_times):] = source_edge_times
        edge_idxs[i, n_neighbors - len(source_edge_idxs):] = source_edge_idxs
        j+=1


    return neighbors, edge_idxs, edge_times, neighbors_unique, neighbors_num,valid_nodes

  def get_layer_community_neighbor(self, source_nodes, timestamps, max_member_num =20, n_neighbors=20, layer = 1):
    """
    Given a list of users ids and relative cut times, extracts a sampled temporal neighborhood of each user in the list.
